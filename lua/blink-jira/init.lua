...
---@field initial_load_complete boolean Flag to track if initial load is done
---@field pending_callbacks function[] Callbacks waiting for initial load
local jira_source = {}
...
function jira_source.new(opts)
	...
	opts.initial_load_complete = false
	opts.pending_callbacks = {}
	...
	-- Mark initial load as complete if cache was loaded
	if cache_loaded then
		instance.initial_load_complete = true
	end
	...
	if opts.background_refresh then
		...
		if not cache_loaded then
			...
			fetch_jira_tickets_async(
				instance,
				instance.jira_project,
				instance.jira_status,
				...
				function(items)
					...
					for _, callback in ipairs(instance.pending_callbacks) do
						callback({
					...
					instance.pending_callbacks = {}
				end
			)
		end
	else
		-- If background refresh is disabled, mark as complete
		instance.initial_load_complete = true
	end
...
function jira_source:get_completions(context, callback)
	...
	local trigger_char = jira_source:get_trigger_characters()[1]
	...
	-- If initial load is not complete, show loading indicator and queue the callback
	if not self.initial_load_complete then
		-- Show loading item immediately
		callback({
			is_incomplete_forward = false,
			is_incomplete_backward = false,
			items = {
				{
					label = "Loading JIRA tickets...",
					insertText = "",
					insertTextFormat = vim.lsp.protocol.InsertTextFormat.PlainText,
					kind = require("blink.cmp.types").CompletionItemKind.Text,
					detail = "Fetching from JIRA API",
					documentation = {
						kind = "markdown",
						value = "⏳ Loading JIRA tickets from API...\n\nThis may take a few seconds on first use.",
					},
				},
			},
		})

		-- Queue the real callback for when data loads
		table.insert(self.pending_callbacks, function(result)
	...
	if
		not self.background_refresh
		and (current_time - self.cache_time > self.cache_duration or #self.completion_items == 0)
	then
